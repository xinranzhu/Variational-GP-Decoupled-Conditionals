...
class VariationalStrategyDecoupledConditionals(_VariationalStrategy):
    ...
    @cached(name="cholesky_factor_schur", ignore_args=True)
    def _cholesky_factor_schur(self, induc_induc_covar):
        L = psd_safe_cholesky(delazify(induc_induc_covar).type(_linalg_dtype_cholesky.value()))
        return TriangularLazyTensor(L)
    ...
    def kl_divergence(self, **kwargs):
        ...
        m_p = full_output.mean[..., :num_induc]
        ...
        mean_kernel = self.covar_module_mean
        induc_induc_covar_mean = mean_kernel(self.inducing_points).add_jitter()
